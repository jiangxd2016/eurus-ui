          ...
          :style="{
            borderTop:
              !isRow && !attrs.reverse && isExpanded
                ? '1px solid var(--vue-run-border, #ebeef5)'
                : ''
          }"
...
import VueRunSfcPreview from "./components/vue-run-preview";
import VueRunSfcHeader from "./components/vue-run-header";
import VueRunSfcControl from "./components/vue-run-control";
import VueRunSfcMain from "./components/vue-run-main";
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      cssVars({
        variables: {
          "--vue-run-main": this.attrs.themeColor || "#409eff",
          "--vue-run-border": this.attrs.themeBorderColor || "#eaeefb"
        }
      });
...
.vue-run-editor .CodeMirror {
  height: inherit;
}
.vue-run-editor .CodeMirror-vscrollbar {
  display: none !important;
}
.vue-run-editor .CodeMirror-scrollbar-filler {
  display: none !important;
}
.vue-run-editor .CodeMirror-sizer {
