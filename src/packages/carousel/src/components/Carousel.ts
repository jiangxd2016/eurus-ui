...
export default defineComponent({
  ...
  setup(props: CarouselConfig, { slots, emit, expose }: SetupContext) {
    ...
    function updateSlidesBuffer(): void {
      const slidesArray = [...Array.from({ length: slidesCount.value }).keys()];
    ...
    function handleDragEnd(): void {
      ...
      const newSlide = getCurrentSlideIndex(
        config,
    ...
    return () => {
      ...
      slidesElements.forEach(
        (el: { props: Record<string, any> }, index: number) => (el.props.index = index)
      );
      const trackEl = h(
        'ol',
        {
