		...
		return () => {
			const Control = () => {
				...
				if (props.multiple && isArray(_value.value)) {
					return (
						<div
							class={`${prefixCls}-control-multiple`}
							role="menu"
							tabindex={0}
			...
			return (
				<div class={computedCls.value} ref={selectDownRef}>
					<div
						class={[
							`${prefixCls}-control`,
							`${prefixCls}-control-${props.size}`,
							isFocus.value && `${prefixCls}-control-focus`,
						]}
						role="listbox"
						tabindex={0}
					...
					>
						...
						<input
							...
							onBlur={handleBlur}
						></input>
						{computedCloseVisible.value && !computedDisabled.value && (
							<Icon name="close" class="clear-icon" size={20} onClick={handleClearClick}></Icon>
						)}
						<Icon
							name="chevronDown"
							class={['down-icon', panelVisible.value && 'translate-icon']}
							size={20}
						></Icon>
					</div>
