...
export const usePagination = (props: TableProps, emit: any) => {
  ...
  const pageSize = computed<number>(() => (isObject(props.pagination) ? props.pagination.pageSize ?? _pageSize.value : _pageSize.value));
  const page = computed(() => (isObject(props.pagination) ? props.pagination.current ?? _page.value : _page.value));
