...
<template>
  <div>
    <e-table ref="tableEl" :data="tableData" :columns="columns">
      <template #control="scope">
        <a @click="delClick(scope)">删除</a>
      </template>
    </e-table>
    <p>
      <e-button @click="allSelect">全选</e-button>
      <e-button @click="unSelect">取消全选</e-button>
      <e-button @click="getSelect">获取所选行</e-button>
      <e-button @click="toggleRowSelection(true)">
        将第2,3条设为选中状态
      </e-button>
      <e-button @click="toggleRowSelection(false)">
        取消第2,3条选中状态
      </e-button>
    </p>
  </div>
...
<template>
  <div>
    <e-table :data="tableData" height="200px" :columns="columns" />
  </div>
</template>
...
<template>
  <div>
    <p>
      <e-button @click="state.showHeader = !state.showHeader">
        显示表头:{{ state.showHeader }}
      </e-button>
      <e-button @click="state.hover = !state.hover">hover高亮:{{ state.hover }}</e-button>
      <e-button @click="state.border = !state.border">显示边框:{{ state.border }}</e-button>
      <e-button @click="state.stripe = !state.stripe">显示斑马线:{{ state.stripe }}</e-button>
      <e-button @click="state.ellipsis = !state.ellipsis">
        溢出省略号:{{ state.ellipsis }}
      </e-button>
      <e-button @click="state.title = !state.title">鼠标滑动过提示:{{ state.title }}</e-button>
      <e-button @click="state.dragLine = !state.dragLine">
        拖动改变列宽时垂直线:{{ state.dragLine }}
      </e-button>
    </p>
    <e-table
      :data="tableData"
...
<template>
  <div>
    <e-table :data="tableData" :has-child="true" :columns="columns">
      <template #control="scope">
...
<template>
  <div>
    <e-table :data="tableData" :row-col-span="rowColSpan" :columns="columns" />
  </div>
</template>
...
<template>
  <div>
    <e-table :data="tableData" :pagination="pagination" :columns="columns" />
  </div>
</template>
...
<script setup lang="jsx">
  ...
  {label: '地址', prop: 'address'},
  {
    label: '操作',
    formatter: () => {
    return <e-button size="mini">删除</e-button>
  }
  }
  ]
</script>
